...
const unsigned char Message03[] = { "Tethealla Gate v.046" };
		...
		case 0x00:
			// Add a guild card here.
			{
				...
				if ( ( ch2 < 40 ) || ( ds_found != -1 ) )
				{
					...
					memcpy (&guild_data[ds_found]->friendname[0], &ship->decryptbuf[0x06], 24);
					memcpy (&guild_data[ds_found]->friendtext[0], &ship->decryptbuf[0x2E], 176);
