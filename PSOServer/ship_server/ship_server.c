...
const unsigned char Message03[] = { "Tethealla Ship v.137" };
...
unsigned char hildebear_table[100000] = {0};
unsigned char rappy_table[100000] = {0};
unsigned char lily_table[100000] = {0};
unsigned char slime_table[100000] = {0};
unsigned char merissa_table[100000] = {0};
unsigned char pazuzu_table[100000] = {0};
unsigned char dorphon_table[100000] = {0};
unsigned char kondrieu_table[100000] = {0};
	...
	for (ch2=0;ch2<num_records;ch2++)
	{
		...
		case 64:
			...
			if ( mm->skin & 0x01 ) // Set rare from a quest?
				...
				if ( ( l->rareIndex < 0x1E ) && ( hildebear_table [ mt_lrand() % 100000 ] ) )
				{
		...
		case 65:
			...
			if ( mm->skin & 0x01 ) // Set rare from a quest?
				...
				if ( ( l->rareIndex < 0x1E ) && ( rappy_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 97:
			...
			if ( ( ( mm->reserved11 - FLOAT_PRECISION ) < (float) 1.00000 ) &&
				...
				if ( ( l->rareIndex < 0x1E ) && ( lily_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 100:
			...
			if ( ( ( mm->reserved11 - FLOAT_PRECISION ) < (float) 1.00000 ) &&
				...
				if ( ( l->rareIndex < 0x1E ) && ( slime_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 274:
			...
			if ( mm->skin & 0x01 ) // Set rare from a quest?
				...
				if ( ( l->rareIndex < 0x1E ) && ( merissa_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 276:
			...
			if ( mm->skin & 0x01 ) // Set rare from a quest?
				...
				if ( ( l->rareIndex < 0x1E ) && ( pazuzu_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 278:
			...
			if ( mm->skin & 0x01 ) // Set rare from a quest?
				...
				if ( ( l->rareIndex < 0x1E ) && ( dorphon_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
		...
		case 281:
			...
			if ( ( ( mm->reserved11 - FLOAT_PRECISION ) < (float) 1.00000 ) &&
				...
				if ( ( l->rareIndex < 0x20 ) && ( kondrieu_table [ mt_lrand() % 100000 ] ) )
				{
					*(unsigned short*) &l->rareData[l->rareIndex] = (unsigned short) l->mapIndex;
					l->rareIndex += 2;
					r = 1;
...
void SendA2 (unsigned char episode, unsigned char solo, unsigned char category, unsigned char gov, BANANA* client)
{
	...
	else
	{
		...
		for (ch=0;ch<qm->quest_counts[category];ch++)
		{
			...
			{
				memcpy (&quest_num[0], &q->qdata[49], 3);
				quest_num[4] = 0;
				qn = atoi (&quest_num[0]);
				if ((ep1solo) || (qn > 26))
					show_quest = 1;
			...
			if (show_quest)
			{
				...
				memcpy (&PacketData[Offset+0x08], &q->qname[0], 0x40);
				memcpy (&PacketData[Offset+0x48], &q->qsummary[0], 0xF0);
				if ((solo) && (episode == 0x01))
				{
					if (qn <= 26)
					{
...
void LogonProcessPacket (ORANGE* ship)
{
	...
	unsigned char* mob_table;
	unsigned mob_rate;
	unsigned mob_counter;
	...
	case 0x10:
		...
		printf ("Building appearance tables...\n");
		for (ch=0;ch<8;ch++)
		{
			...
			case 0x00:
				...
				mob_table = &hildebear_table[0];
				break;
			case 0x01:
				printf ("Rappy appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &rappy_table[0];
				break;
			case 0x02:
				printf ("Lily appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &lily_table[0];
				break;
			case 0x03:
				printf ("Pouilly Slime appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &slime_table[0];
				break;
			case 0x04:
				printf ("Merissa AA appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &merissa_table[0];
				break;
			case 0x05:
				printf ("Pazuzu appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &pazuzu_table[0];
				break;
			case 0x06:
				printf ("Dorphon Eclair appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &dorphon_table[0];
				break;
			case 0x07:
				printf ("Kondrieu appearance rate: %3f%%\n", (float) mob_rate / 1000 );
				mob_table = &kondrieu_table[0];
				break;
			}
			if ( mob_rate )
			{
				mob_rate = 100000 / mob_rate;
				mob_counter = 0;
				for (ch2=0;ch2<100000;ch2++)
				{
					mob_counter++;
					if (mob_counter >= mob_rate)
					{
						mob_table[ch2] = 1;
						mob_counter = 0;
					}
				...
				memset ( mob_table, 0, 100000 ); // No rate set...
		}
...
void CheckMagEvolution ( MAG* m, unsigned char sectionID, unsigned char type, int skip_evolution )
{
	short mDefense, mPower, mDex, mMind;
	int typeB = 0;
	...
	if ( !skip_evolution )
	{
		if ( ( sectionID % 2 ) && ( sectionID != 0 ) )
			typeB = 1; // Odd number IDs are Type B
		mDefense = m->defense / 100;
		mPower = m->power / 100;
		mDex = m->dex / 100;
		mMind = m->mind / 100;
		...
		if ( m->level == 35 )
		{
			...
			case CLASS_HUCAST:
			case CLASS_HUCASEAL:
				if ( ( mPower >= mDex ) && ( mPower >= mMind ) )
				{
					m->mtype = Mag_Rudra;
					AddPB ( &m->PBflags, &m->blasts, PB_Golla);
					return;
				}
				if ( ( mDex >= mPower ) && ( mDex >= mMind ) )
				{
					m->mtype = Mag_Marutah;
					AddPB ( &m->PBflags, &m->blasts, PB_Pilla);
					return;
				}
				if ( ( mMind >= mPower ) && ( mMind >= mDex ) )
				{
					m->mtype = Mag_Vayu;
		...
		if ( ( m->level >= 50 ) && ( ( m->level % 5 ) == 0 ) )
		{
			// Special Evolution check before 3rd evolution check
			if ( ( m->level >= 100 ) && ( ( m->level % 10 ) == 0 ) )
			{
				switch ( sectionID )
				{
				case ID_Viridia:
			...
			// Force special mag check

			if ( ( type == CLASS_FONEWM ) || ( type == CLASS_FONEWEARL ) ||
				 ( type == CLASS_FOMARL ) || ( type == CLASS_FOMAR ) )
			{
				if ( ( mDefense >= 45 ) && ( mPower >= ( mDex + mMind ) ) )
				{
					m->mtype = Mag_Andhaka;
					AddPB ( &m->PBflags, &m->blasts, PB_Estlla);
					return;
				...
				if ( ( mDefense >= 45 ) && ( mMind >= ( mDex + mPower ) ) )
				{
					m->mtype = Mag_Bana;
					AddPB ( &m->PBflags, &m->blasts, PB_Estlla);
					return;
				...
				if ( ( mDefense >= 45 ) && ( mDex >= ( mPower + mMind ) ) )
				{
					m->mtype = Mag_Bana;
					AddPB ( &m->PBflags, &m->blasts, PB_Estlla);
					return;
			...
			case 0x00:
				...
				case CLASS_HUCAST:
				case CLASS_HUCASEAL:
					if ( ( mPower >= mDex ) && ( mDex >= mMind ) )
					{
						m->mtype = Mag_Varaha;
						AddPB ( &m->PBflags, &m->blasts, PB_Golla);
						return;
					}
					if ( ( mPower >= mMind ) && ( mMind >= mDex ) )
					{
						m->mtype = Mag_Bhirava;
						AddPB ( &m->PBflags, &m->blasts, PB_Pilla);
						return;
					}
					if ( ( mDex >= mPower ) && ( mPower >= mMind ) )
					{
						m->mtype = Mag_Ila;
						AddPB ( &m->PBflags, &m->blasts, PB_Mylla_Youlla);
						return;
					}
					if ( ( mDex >= mMind ) && ( mMind >= mPower ) )
					{
						m->mtype = Mag_Nandin;
						AddPB ( &m->PBflags, &m->blasts, PB_Estlla);
						return;
					}
					if ( ( mMind >= mPower ) && ( mPower >= mDex ) )
					{
						m->mtype = Mag_Kabanda;
						AddPB ( &m->PBflags, &m->blasts, PB_Mylla_Youlla);
						return;
					}
					if ( ( mMind >= mDex ) && ( mDex >= mPower ) )
					{
						m->mtype = Mag_Ushasu;
						AddPB ( &m->PBflags, &m->blasts, PB_Golla);
						return;
...
void FeedMag (unsigned magid, unsigned itemid, BANANA* client)
{
	...
	else
	{
		...
		if ( mDefense >= 100 )
		{
			...
			CheckMagEvolution ( m, client->character.sectionID, client->character._class, skip_evolution );
		}
		else
			m->defense  = ( ( m->defense / 100 ) * 100 ) + mDefense;
...
void Send60 (BANANA* client)
{
	...
	if (!dont_send)
	{
		...
		case 0x29:
			if ( ( client->lobbyNum > 0x0F ) && ( client->decryptbuf[0x0A] == client->clientID ) )
			{
				// Client splitting a stack of an item
				itemid = *(unsigned *) &client->decryptbuf[0x0C];
				count = *(unsigned *) &client->decryptbuf[0x10];
				if (itemid != 0xFFFFFFFF)
					DeleteItemFromClient (itemid, count, 1, client); // Drop on the ground
				else
					DeleteMesetaFromClient (count, 1, client); // Inflation doesn't stand a chance! </joke>
			}
			else
			{
				dont_send = 1;
							...
							if (lClient->mode == 0x02)
							{
								for (ch2=0;ch2<lClient->character.inventoryUse;ch2++)
								{
									if (lClient->character.inventory[ch2].item.data[0] == 0x02)
										DeleteItemFromClient (lClient->character.inventory[ch2].item.itemid, 1, 0, lClient);
								}
			...
			if (!l->drops_disabled)
			{
				...
				if ( ( mid < 0xB50 ) && ( l->boxHit[mid] == 0 ) )
				{
					...
					if (!box_rare)
					{
						...
						for (ch=0;ch<30;ch++)
						{
							if (*rt_table == l->floor[client->clientID])
							{
		...
		if ( client->lobbyNum > 0x0F )
		{
			...
			for (ch=0;ch<client->character.inventoryUse;ch++)
			{
					...
					(client->character.meseta >= 100))
				{
					...
					memcpy (&client->tekked.item.data[0], &i->item.data[0], 16);
					if ( attrib < 0x29)
					{
			...
			for (ch=0;ch<4;ch++)
			{
				if ((l->slot_use[ch]) && (l->client[ch]))
				{
					for (ch2=0;ch2<l->client[ch]->character.inventoryUse;ch2++)
						...
						{
							Send1A ("Possible dupe attempt.", client);
							client->todc = 1;
							break;
						}
				}
...
void Command09(BANANA* client)
{
			...
			if (l->in_use)
			{
				...
				else
				{
					...
					if (l->quest_loaded)
					{
						...
						PacketData[Packet11_Length++] = 0x00;
						q = &quests[l->quest_loaded - 1];
						wstrcpy((unsigned short*) &PacketData[Packet11_Length], (unsigned short*) &q->qname[0]);
						Packet11_Length += wstrlen ((unsigned short*) &PacketData[Packet11_Length]);
					}
				}
			}
	...
	case 0x0F:
		...
		if ( ( client->lobbyNum > 0x0F ) && ( client->decryptbuf[0x0B] <= numQuests ) )
		{
			...
			memcpy (&PacketData[0x08], &q->qdetails[0], 0x200 );
			cipher_ptr = &client->server_cipher;
													...
													while (1)
													{
														for (ch2=0;ch2<l->client[ch]->character.inventoryUse;ch2++)
														{
															if (l->client[ch]->character.inventory[ch2].item.data[0] == 0x02)
																magFound = l->client[ch]->character.inventory[ch2].item.itemid;
														}
														if (magFound)
															DeleteItemFromClient (magFound, 1, 0, l->client[ch]);
														else
															break;
													}
	...
	if (client->guildcard)
	{
		...
		case 0x44:
			if ( ( client->lobbyNum > 0x0F ) && ( client->sending_quest != -1 ) )
			{
				...
				if (client->qpos < quests[client->sending_quest].qsize)
				{
					qps = *(unsigned short*) &quests[client->sending_quest].qdata[client->qpos];
					if ( qps % 8 )
						qps += ( 8 - ( qps % 8 ) );
					cipher_ptr = &client->server_cipher;
					encryptcopy (client, &quests[client->sending_quest].qdata[client->qpos], qps);
					client->qpos += qps;
				}
				else
		...
		case 0x6F:
			if ((client->lobbyNum > 0x0F) && (client->bursting))
			{
				...
				if ((l->inpquest) && (!client->hasquest))
				{
					// Send the quest
					client->bursting = 1;
					fqs =  *(unsigned short*) &quests[l->quest_loaded - 1].qdata[0];
					if (fqs % 8)
						fqs += ( 8 - ( fqs % 8 ) );
					client->sending_quest = l->quest_loaded - 1;
					client->qpos = fqs;
					cipher_ptr = &client->server_cipher;
					encryptcopy ( client, &quests[l->quest_loaded - 1].qdata[0], fqs);
				}
				else
				{
	...
	while (fgets (&qfile[0], 255, fp) != NULL)
	{
		...
		while (fgets (&qfile2[qf2l], 255, qf) != NULL)
		{
			...
			if (qd != NULL)
			{
				...
				q->qdata = malloc ( qs );
				questsMemory += qs;
				fread ( &q->qdata[0], 1, qs, qd );
				ch = 0;
				ch2 = 0;
				while (ch < qs)
				{
					qpc = *(unsigned short*) &q->qdata[ch+2];
					if ( (qpc == 0x13) && (strstr(&q->qdata[ch+8], ".bin")) && (ch2 < PRS_BUFFER))
					{
						memcpy ( &true_filename[0], &q->qdata[ch+8], 16 );
						qps2 = *(unsigned*) &q->qdata[ch+0x418];
						memcpy (&qpd_buffer[ch2], &q->qdata[ch+0x18], qps2);
						ch2 += qps2;
					}
					else
						if (ch2 >= PRS_BUFFER)
				...
				qname[qnl] = 0;
				memcpy (&q->qname[0], &qpdc_buffer[0x18], 0x40);
				q->qname[qnl] = 0x0000;
				memcpy (&q->qsummary[0], &qpdc_buffer[0x58], 0x100);
				memcpy (&q->qdetails[0], &qpdc_buffer[0x158], 0x200);
				...
				{
					qpc = *(unsigned short*) &q->qdata[ch+2];
					if ( (qpc == 0x13) && (strstr(&q->qdata[ch+8], ".dat")) && (ch2 < PRS_BUFFER))
					{
						qps2 = *(unsigned *) &q->qdata[ch+0x418];
						memcpy (&qpd_buffer[ch2], &q->qdata[ch+0x18], qps2);
						ch2 += qps2;
...
/*
				...
				*/
				...
				printf ("File %s is missing.  Could not load the quest.\n", qfile2);
		}
		fclose (qf);
		qm->num_categories++;
	}
